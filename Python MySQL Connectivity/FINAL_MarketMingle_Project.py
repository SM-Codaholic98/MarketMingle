...
def CREATE_TABLE():
    sql01 = 'USE MARKET'
    sql02 = 'CREATE TABLE Shop(S_Id VARCHAR(10) NOT NULL PRIMARY KEY, SName VARCHAR(50), Area VARCHAR(30), City VARCHAR(30), State CHAR(30), ESTD DATE)'
    sql03 = 'CREATE TABLE Accessories(A_Id VARCHAR(10) NOT NULL PRIMARY KEY, AName VARCHAR(50), Price DECIMAL(60, 2), S_Id VARCHAR(10), FOREIGN KEY (S_Id) REFERENCES Shop(S_Id))'
...
def INSERT_Shop_Records():
    ...
    while ans.lower()=='y':
        ...
        data = (S_Id, SName, Area, City, State, ESTD)
...
def INSERT_Accessories_Records():
    ...
    while ans.lower()=='y':
        ...
        S_Id = input("Enter the shop ID : ")
        data = (A_Id, AName, Price, S_Id)
        sql = "INSERT INTO Accessories(A_Id, AName, Price, S_Id) VALUES(%s,%s,%s,%s)"
        c = db.cursor()
        c.execute(sql, data)
        db.commit()
...
def DISPLAY_Shop_Table():
    ...
    while ans.lower()=='y':
        ...
        for i in rows:
            ...
            print("City = ", i[3])
            print("State = ", i[4])
            print("Shop Establishment Date = ", i[5])
            print("-----------------------------------")
            print()
...
def UPDATE_Accessories_Price():
    ...
    while ans.lower()=='y':
        ...
        if CHECK_Accessories_Record(id)==False:
            ...
            amount = input("Enter the updated price of the product : ")
            amount = float(decimal.Decimal(amount))
            sql = "SELECT Price FROM ACCESSORIES WHERE A_Id=%s"
            data = (id,)
            c = db.cursor()
            c.execute(sql, data)
            price = c.fetchone()
            p = float(decimal.Decimal(price[0]))
            new_price = p + amount
            new_price = str(new_price)
            
            sql = "UPDATE Accessories SET Price=%s WHERE A_Id=%s"
            data = (new_price, id)
            c.execute(sql, data)
            db.commit()
            print("*** Product's price has been successfully updated in the ACCESSORIES table ***")
        print()
...
while ans.lower()=='y':
    ...
    print("Press 7 to update the price of a product in the ACCESSORIES table")
